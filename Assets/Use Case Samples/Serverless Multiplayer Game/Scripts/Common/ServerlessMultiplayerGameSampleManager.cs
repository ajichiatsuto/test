...
namespace Unity.Services.Samples.ServerlessMultiplayerGame
{
    ...
    public class ServerlessMultiplayerGameSampleManager : MonoBehaviour
    {
        ...
        void Awake()
        {
            Debug.Log("ServerlessMultiplayerGameSampleManager.Awake()");
        ...
        async void Start()
        {
            Debug.Log("ServerlessMultiplayerGameSampleManager.Start()");
            try
            {
        ...
        {
            Debug.Log($"ServerlessMultiplayerGameSampleManager.SignInAndInitialize({profileDropdownIndex})");
            try
            {
        ...
        {
            Debug.Log($"ServerlessMultiplayerGameSampleManager.ValidatePlayerName({playerName})");
            try
            {
        ...
        {
            Debug.Log($"ServerlessMultiplayerGameSampleManager.SetReturnToMenuReason({returnToMenuReason})");
            this.returnToMenuReason = returnToMenuReason;
        }

        public void ClearReturnToMenuReason()
        {
            Debug.Log("ServerlessMultiplayerGameSampleManager.ClearReturnToMenuReason()");
            this.returnToMenuReason = ReturnToMenuReason.None;
        }

        public void SetPreviousGameResults(GameResultsData results)
        {
            Debug.Log($"ServerlessMultiplayerGameSampleManager.SetPreviousGameResults({results})");
        ...
        {
            Debug.Log("ServerlessMultiplayerGameSampleManager.ClearPreviousGameResults()");
            arePreviousGameResultsSet = false;
        }

        void OnDestroy()
        {
            Debug.Log("ServerlessMultiplayerGameSampleManager.OnDestroy()");
            if (instance == this)
            {
                instance = null;
            }
